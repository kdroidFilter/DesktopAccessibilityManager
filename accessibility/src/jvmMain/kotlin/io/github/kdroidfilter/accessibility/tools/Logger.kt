package io.github.kdroidfilter.accessibility.tools

var allowDesktopAccessibilityManagerLogging: Boolean = false
...
fun debugln(message: () -> String) {
    if (allowDesktopAccessibilityManagerLogging && loggingLevel.priority <= LoggingLevel.DEBUG.priority) {
        println(message())
    }
}

fun verboseln(message: () -> String) {
    if (allowDesktopAccessibilityManagerLogging && loggingLevel.priority <= LoggingLevel.VERBOSE.priority) {
...
fun infoln(message: () -> String) {
    if (allowDesktopAccessibilityManagerLogging && loggingLevel.priority <= LoggingLevel.INFO.priority) {
        println(message(), COLOR_AQUA)
    }
}

fun warnln(message: () -> String) {
    if (allowDesktopAccessibilityManagerLogging && loggingLevel.priority <= LoggingLevel.WARN.priority) {
        println(message(), COLOR_ORANGE)
    }
}

fun errorln(message: () -> String) {
    if (allowDesktopAccessibilityManagerLogging && loggingLevel.priority <= LoggingLevel.ERROR.priority) {
